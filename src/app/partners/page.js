    ...
    <main>
      ...
      <section className="py-[60px] md:py-[90px] relative bg-[#f8fafc] text-[var(--navy)]">
        <div className="d-inner">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-[52px]">
            {[
              ...
              <Card className="h-full border border-[var(--border)] shadow-sm hover:shadow-md transition-shadow duration-300 bg-white text-left" key={i}>
                <CardHeader className="pb-3">
                  <div className="font-[family-name:var(--fd)] text-[12px] font-bold text-[var(--cyan)] tracking-[2px] uppercase mb-1">{w.num}</div>
                  <CardTitle className="text-[18px] font-extrabold text-[var(--navy)] leading-tight">{w.title}</CardTitle>
                </CardHeader>
                <CardContent>
                  <CardDescription className="text-[14px] text-[var(--muted)] leading-[1.75]">{w.desc}</CardDescription>
                </CardContent>
              </Card>
            ))}
          </div>
      ...
      <section className="py-[60px] md:py-[90px] relative bg-gradient-to-br from-[#07102a] to-[#06091a] text-white">
        <div className="d-inner">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-12">
            {[
              { ico: <Handshake size={32} strokeWidth={1.5} className="text-[var(--cyan)]" />, title: 'Co-Innovation & Solution Building', desc: "Co-create new differentiated solutions using Hexatech's eHEX platform and domain accelerators. Access our product roadmap, early-release APIs, and dedicated co-innovation labs to build IP that wins new markets together." },
              { ico: <Rocket size={32} strokeWidth={1.5} className="text-[var(--cyan)]" />, title: 'Go-to-Market Enablement', desc: 'Joint campaigns, partner playbooks, reference architectures, and sales enablement support. Our partner marketing team co-invests in demand generation, event presence, and analyst recognition programmes on your behalf.' },
              { ico: <Zap size={32} strokeWidth={1.5} className="text-[var(--cyan)]" />, title: 'Faster Delivery & Repeatability', desc: 'Deploy transformation projects faster with composable architecture and modular building blocks. Certified delivery frameworks, pre-built accelerators, and rapid-implementation playbooks reduce delivery risk and time-to-value.' },
              { ico: <ShieldCheck size={32} strokeWidth={1.5} className="text-[var(--cyan)]" />, title: 'Enterprise Readiness by Design', desc: "Hexatech's scalable and governance-built platform is built for the needs of regulated institutions. Security, compliance, audit, and resilience are embedded by design — not added after the fact." },
              { ico: <TrendingUp size={32} strokeWidth={1.5} className="text-[var(--cyan)]" />, title: 'Commercial & Growth Frameworks', desc: 'Structured incentives, co-opetition engagement frameworks, and joint revenue opportunities. Tier-based partner programmes reward investment in certification, pipeline, and delivery with higher margins and priority access.' },
            ].map((g, i) => (
              <Card className="bg-white/5 border-white/10 text-white hover:border-[rgba(0,200,232,0.3)] hover:-translate-y-1 hover:bg-[rgba(0,200,232,0.03)] transition-all duration-300 text-left" key={i}>
                <CardHeader className="pb-2">
                  <div className="w-10 h-10 rounded-[10px] bg-[rgba(0,200,232,0.1)] border border-[rgba(0,200,232,0.2)] flex items-center justify-center text-[18px] mb-2">{g.ico}</div>
                  <CardTitle className="font-[family-name:var(--fd)] text-[16px] font-bold text-white leading-tight">{g.title}</CardTitle>
                </CardHeader>
                <CardContent>
                  <CardDescription className="text-[14px] text-white/60 leading-[1.7]">{g.desc}</CardDescription>
                </CardContent>
              </Card>
            ))}
            <Card className="bg-gradient-to-br from-[#00c8e8] to-white border border-[rgba(0,200,232,0.25)] transition-all duration-300 relative overflow-hidden text-left hover:border-[rgba(0,200,232,0.25)] hover:-translate-y-1">
